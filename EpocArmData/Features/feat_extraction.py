...
from Library.pyeeg import pfd,hjorth_com,hjorth_mob
...
emg_channel = ['AF3','F8'];emg_folder = ['Left_Wink','Right_Wink']
classes = {'active','standard','offline'}
classes_b = {'active','standard','offline'}
...
train_n_beta = 128; total_n_beta = 192;data_n_beta = 128
test_n_beta = 64
numClss = 6;
Dists = ['Active','Standard', 'Offline']
Dists_b = ['Active', 'Standard', 'Offline', 'EOG','EMG-Wink']
Dists_emg = ['Active', 'Standard', 'Offline', 'EOG']
seq_ = [];seq_b = []; seq_m = []
Csvm_wr = 100; Gsvm_wr = 0.1
Csvm_wl = 100; Gsvm_wl = 0.1
Csvm_al = 100; Gsvm_al = 0.1
Csvm_be = 100; Gsvm_be = 0.1
...
def parametrs_alpha(val_):
	...
	vaf_tt = {'a1_ratio':vaf_r,'hjr_mob':vaf_hjorth_mob}
	return vaf_tt
#--------------------------- Beta Parameters --------------------------
...
#	vaf_hjorth_com = (hjorth_com(val_))/100.0
	vaf_hjorth_com = pfd(val_)
	vaf_tt = {'a1_ratio':vaf_r,'hjr_com':vaf_hjorth_com,'hjr_mob': vaf_hjorth_mob}
	return vaf_tt
#--------------------------- EMG Parameters ----------------------------
...
def alpha_extra_():
	...
	for j, dir_name in enumerate(dic_fol):
		exec "ch_ = loadmat_new.loadmat('%s/channels/Alpha/Test_1_A/%s/O2.mat')['O2']" %(dir_wrk,dir_fol_alpha[j])
		len_st = [0,len(ch_),2*len(ch_),3*len(ch_),4*len(ch_),5*len(ch_)]
		exec "st_ = loadmat_new.loadmat('%s/channels/Beta/Standard/O2.mat')['O2']" %(dir_wrk)
		st_ = st_[len_st[j]:len_st[j+1]]
		exec "of_ = loadmat_new.loadmat('%s/channels/Beta/Offline/O2.mat')['O2']" %(dir_wrk)
...
def beta_extra_():
	...
	exec "ch_ = loadmat_new.loadmat('%s/channels/Beta/Test_1_B/AF3.mat')['AF3']" %(dir_wrk)
	exec "st_ = loadmat_new.loadmat('%s/channels/Beta/Standard/AF3.mat')['AF3']" %(dir_wrk)
	exec "of_ = loadmat_new.loadmat('%s/channels/Beta/Offline/AF3.mat')['AF3']" %(dir_wrk)
	exec "eo_ = loadmat_new.loadmat('%s/channels/EMG/EOG/AF3.mat')['AF3']" %(dir_wrk)
	exec "em_ = loadmat_new.loadmat('%s/channels/EMG/Left_Wink/AF3.mat')['AF3']" %(dir_wrk)
	#channel --
	ch_ = dc2uV(ch_)
	ch_ = filtering(ch_)
	ch_ = np.split(ch_,32)
...
def emg_extra_():
	for xx,yy in enumerate(emg_folder):
		...
		exec "ch_ = loadmat_new.loadmat('%s/channels/EMG/%s/%s.mat')['%s']" %(dir_wrk,emg_folder[xx],emg_channel[xx],emg_channel[xx])
		exec "eo_ = loadmat_new.loadmat('%s/channels/EMG/EOG/%s.mat')['%s']" %(dir_wrk,emg_channel[xx],emg_channel[xx])
		exec "st_ = loadmat_new.loadmat('%s/channels/Beta/Standard/%s.mat')['%s']" %(dir_wrk,emg_channel[xx],emg_channel[xx])
		exec "of_ = loadmat_new.loadmat('%s/channels/Beta/Offline/%s.mat')['%s']" %(dir_wrk,emg_channel[xx],emg_channel[xx])
		st_ = st_[0:len(ch_)]
		of_ = of_[0:len(ch_)]
		#standard --
		st_ = dc2uV(st_)
...
def plot_alpha_():
	...
	for y,cls in enumerate(classes):
		for x in range(total_n_alfa):
			...
			exec "%s_hjr.append(%s[seq_[%s]]['hjr_mob'])" % (cls,cls,x)
	#-------------------------------------------------------------------
	data1 = [active_ratio, standard_ratio, offline_ratio]
	data2 = [active_hjr, standard_hjr, offline_hjr]
	#-------------------------------------------------------------------
	plt.ion()
...
def plot_beta_():
	...
	#-------------------------------------------------------------------
 	for y,cls in enumerate(classes_b):
		for x in range(total_n_beta):
			exec "%s_ratio.append(%s[seq_b[%s]]['a1_ratio'])" % (cls,cls,x)
			exec "%s_com.append(%s[seq_b[%s]]['hjr_com'])" % (cls,cls,x)
			exec "%s_mob.append(%s[seq_b[%s]]['hjr_mob'])" % (cls,cls,x)
	for y,cls in enumerate(classes_ext):
		for x in range(total_n_emg):
			...
			exec "%s_mob.append(%s[seq_b[%s]]['hjr_mob'])" % (cls,cls,x)
	#-------------------------------------------------------------------
	data1 = [active_ratio, standard_ratio, offline_ratio, eog_ratio, emg_ratio]
	data2 = [active_com, standard_com, offline_com, eog_com, emg_com]
	data3 = [active_mob, standard_mob, offline_mob, eog_mob, emg_mob]
	#-------------------------------------------------------------------
...
def svm_alpha_():
	#------------------------------ labels -----------------------------
	# data full
	target_names = np.array(['active','standard','offline'])
	# data 2D
	target_names_2d = np.array(['active','standard'])
	feature_names_2d = ['a1_ratio','hjr_mob']
	#------------------------------ Variables ------------------------------
	seq_ = []
	...
	# Train part
	X_2d = np.delete(X,range(50,70)+range(100,130),axis=0)
	y_2d = y[y < 2]
	# Test part
...
def svm_beta_():
	#------------------------------ labels -----------------------------
	# data full
	target_names_all = np.array(['active','standard','offline','eog','emg'])
	target_names = np.array(['active','standard','offline'])
	...
	#------------------------------ Data Binary Class and Feats --------
	# Train part
	X_2d = np.delete(X,range(128,192)+range(256,360)+range(384,414)+range(434,464),axis=0)
	y_2d = y[y < 2]
	# Test part
	T_2d = np.delete(T,range(64,96)+range(128,180)+range(192,207)+range(217,232),axis=0)
	yy_2d = yy[yy < 2]
	#------------------------------ Standardize data ------------
	scaler = StandardScaler()
	X_2d = scaler.fit_transform(X_2d)
	T_2d_scaled = scaler.transform(T_2d)
	#------------------------------ Create Classifier ------------------
	manual_param = {'C':Csvm_be,'gamma':Gsvm_be}
	clf = SVC(gamma=manual_param['gamma'], C=manual_param['C'])
	clf.fit(X_2d, y_2d)
	#------------------------------ Create Classifier ------------------
	C_range = np.logspace(1, 3, 3)
	...
	ax.scatter(X_2d[:, 0], X_2d[:, 1], X_2d[:, 2], c=y_2d, marker='o')
...
def svm_wleft_():
	...
	T_2d_scaled = scaler.transform(T_2d)
	#------------------------------ Create Classifier ------------------
	manual_param = {'C':Csvm_wr,'gamma':Gsvm_wl}
	clf = SVC(gamma=manual_param['gamma'], C=manual_param['C'])
	clf.fit(X_2d, y_2d)
	#------------------------------ Create Classifier ------------------
	C_range = np.logspace(1, 3, 3)
...
def svm_wright_():
	...
	T_2d_scaled = scaler.transform(T_2d)
	#------------------------------ Create Classifier ------------------
	manual_param = {'C':Csvm_wr,'gamma':Gsvm_wr}
	clf = SVC(gamma=manual_param['gamma'], C=manual_param['C'])
	clf.fit(X_2d, y_2d)
	#------------------------------ Create Classifier ------------------
	C_range = np.logspace(1, 3, 3)
